...
async def init_elo_ranks():
    ...
    for rank in EloRank.get_all():
        ...
        try:
            ...
            await d_obj.d_log(f"Couldn't create emoji for {rank.name} role, Emoji limit exceeded!")

        if not rank.role.icon and emoji and not cfg.TEST:  # Disable setting icons in test mode, no boosts
            try:
                await rank.role.edit(icon=await emoji.read())
