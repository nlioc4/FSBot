    ...
    async def anomaly_check(self):
        async with auraxium.Client(service_id=cfg.general['api_key']) as client:
            ...
            for event in self.active_events:
                if (not event.message) and event.timestamp <= (datetime.now() - timedelta(minutes=25)):
                    event.message = await self.notif_channel.send(content=f'{self.notif_role.mention}',
                ...
                elif event.message and event.state_id == 138:
        ...
        match action:
            case "Start":
                if self.anomaly_check.is_running():
                    await ctx.respond(f"Anomaly Check is running with role:{self.notif_role} and channel: {self.notif_channel}", ephemeral=True)
                else:
                    await ctx.respond(f"Anomaly Check started with role:{self.notif_role} and channel: {self.notif_channel}", ephemeral=True)
                    self.anomaly_check.start()
            case "Stop":
                if self.anomaly_check.is_running():
